<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator" :md="24" :sm="24" style="margin-top: -15px">
      ...
      <a-button @click="handleAddUserDepart" type="primary" style="margin-left: 5px" icon="plus">新增</a-button>
    ...
    <!-- table区域-end -->
    <menu-auth-modal ref="menuAuthModal" @ok="loadData"></menu-auth-modal>
  </a-card>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          list: "/system/menuApi/getMenuApis",
          edit: "/sys/user/editSysDepartWithUser",
          delete: "/system/menuApi/delete",
          deleteBatch: "/system/menuApi/batchDelete",
        }
      }
    },
    created() {
    ...
    methods: {
      ...
      batchDel: function () {
        ...
        if (!this.currentDeptId) {
          this.$message.error("未选中任何菜单，无法删除该api!")
          return
        }
      ...
      handleAddUserDepart(record) {
        if (this.currentDeptId == '' ) {
          ...
          this.$refs.menuAuthModal.add(news)
          this.$refs.menuAuthModal.title = '新增'
        }
      },
      selectOK(data) {
        let params = {};
